...
+function ($) {
  $.fn.autocomplete = function (options) {
    ...
    return this.each(function () {
        ...
        if (!resize) {
          ...
          if (that.nav.suggestionHeight) {
            if (!next) {
              ...
              var scrTop = that.nav.scrollTop(),
                  selTop = next.offset().top - that.nav.offset().top;
      ...
      that.nav.on('click', '.prev', $.proxy(clickPrev, this)).on('click', '.first', $.proxy(clickFirst, this)).on('click', '.next', $.proxy(clickNext, this)).on('click', '.last', $.proxy(clickLast, this));
...
+function ($, moment) {
  ...
  var DatePicker = function DatePicker(element, options) {
    ...
    this.container.find('.calendar').on('click.datepicker', '.prev', $.proxy(this.clickPrev, this)).on('click.datepicker', '.next', $.proxy(this.clickNext, this)).on('mousedown.datepicker', 'td.available', $.proxy(this.clickDate, this));
  ...
  DatePicker.prototype.setStartDate = function (startDate) {
    ...
    if ((typeof startDate === 'undefined' ? 'undefined' : _typeof(startDate)) === 'object') {
  ...
  DatePicker.prototype.setEndDate = function (endDate) {
    ...
    if ((typeof endDate === 'undefined' ? 'undefined' : _typeof(endDate)) === 'object') {
  ...
  DatePicker.prototype.updateMonthsInView = function () {
    if (this.endDate) {
      ...
      if (!this.singleDatePicker && this.calendar[0].month && this.startDate.format('YYYY-MM') == this.calendar[0].month.format('YYYY-MM')) {
        for (var i = 1; i < this.numberOfMonths; i++) {
          if (this.calendar[i].month && this.endDate.format('YYYY-MM') == this.calendar[i].month.format('YYYY-MM')) {
            return;
          }
        }
      }
...
+function ($) {
  ...
  var language = (nav.language || nav.browserLanguage || nav.systemLanguage || nav.userLanguage || '').split('-')[0].split('_')[0];
...
+function ($) {
  ...
  var Select = function Select(element) {
    ...
    var $button = $('<button class="btn btn-default select-toggle" type="button" data-toggle="select"/>').append(this.$text) // default value button
  ...
  Select.prototype.toggle = function (e) {
    ...
    if (!$parent.hasClass('open')) {
      if ('ontouchstart' in document.documentElement && !$parent.closest('.navbar-nav').length) {
        // if mobile we use a backdrop because click events don't delegate
        $(document.createElement('div')).addClass('select-backdrop').insertAfter($(this)).on('click', clearMenus);
      }
      ...
      $parent.toggleClass('open').trigger($.Event('shown.bs.select', relatedTarget));
    }
  ...
  $(document).on('click.bs.select.data-api', clearMenus).on('click.bs.select.data-api', '.select form', function (e) {
    e.stopPropagation();
  }).on('click.bs.select.data-api', toggle, Select.prototype.toggle).on('keydown.bs.select.data-api', toggle, Select.prototype.keydown).on('keydown.bs.select.data-api', '.select-options', Select.prototype.keydown);
}(jQuery);
